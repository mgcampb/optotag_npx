...
paths = struct;
paths.npx_data = 'E:\npx_data\with_cam\';
paths.save = 'E:\optotag_tests\salt\';
if ~isfolder(paths.save)
    mkdir(paths.save);
end

opt = struct;
opt.session = 'MC27_20211215.mat';
...
trial_type = SessionData.TrialTypes;
...
parfor cIdx = 1:numel(cellid)
    ...
    salt_p_this = nan(nPulse,nTrialTypes);
    for ttIdx = 1:nTrialTypes
        ...
        for pIdx = 1:nPulse
            ...
            salt_p_this(pIdx,ttIdx) = salt(spt_baseline,spt_test(:,winIdx),opt.dt,opt.wn);
        end   
    end

    salt_p(cIdx,:,:) = salt_p_this;
...
%% save results
save(fullfile(paths.save,opt.session),'salt_p','opt','cellid');


%% make figs
if opt.make_psth
    for cIdx = 1:numel(cellid)
        spiket_ms = 1000 * sp.st(sp.clu==cellid(cIdx));
        figure;
